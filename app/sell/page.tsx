        ...
        <section className="grid gap-8 lg:grid-cols-[1.6fr_0.8fr]">
          <div className="rounded-3xl border border-zinc-800 bg-zinc-950 p-6">
            <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              ...
              <input
                type="text"
                placeholder="Search by name, set, rarity, or card number..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="w-full rounded-2xl border border-zinc-800 bg-black px-4 py-3 text-white outline-none focus:border-yellow-400 md:max-w-md"
              />
            </div>
            ...
            {loading ? (
              ...
              <div className="rounded-2xl border border-zinc-800 bg-black p-10 text-center text-zinc-400">
                No buylist cards found.
              </div>
            ) : (
              <div className="space-y-4">
                {filteredItems.map((item) => {
                  const selected = selectedCards[item.id];
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col gap-5 md:flex-row">
                        <div className="relative h-36 w-28 overflow-hidden rounded-xl bg-zinc-900">
                          <Image
                            src={item.imageUrl}
                            alt={item.name}
                            fill
                          ...
                          <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                            <div>
                              ...
                              <div className="mt-1 space-y-1 text-sm text-zinc-400">
                                {item.setName && <p>Set: {item.setName}</p>}
                                {item.cardNumber && (
                                  <p>Card No: {item.cardNumber}</p>
                                )}
                          ...
                          <div className="mt-5 grid gap-4 md:grid-cols-[1fr_auto] md:items-end">
                            <div>
                              ...
                              <div className="flex gap-2">
                                <button
                                  type="button"
                                  className="rounded-xl border border-yellow-400 bg-yellow-500 px-4 py-3 text-sm font-semibold text-black"
                                >
                                  Mint
                                </button>
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2">
                                <button
                                  ...
                                  className="h-10 w-10 rounded-xl bg-zinc-800 text-lg transition hover:bg-zinc-700"
                                >
                                  -
                                </button>
                                ...
                                <input
                                  ...
                                  className="w-20 rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2 text-center text-white outline-none focus:border-yellow-400"
                                />
          ...
          <aside className="h-fit rounded-3xl border border-zinc-800 bg-zinc-950 p-6">
            ...
            {selectedItemsDetailed.length === 0 ? (
              <p className="mt-4 text-sm leading-7 text-zinc-400">
                Add cards from the buylist to build your submission.
              </p>
            ) : (
              <>
                <div className="mt-5 space-y-4">
                  {selectedItemsDetailed.map((entry) => (
                    <div
                      key={entry.buylistItemId}
                      className="rounded-2xl border border-zinc-800 bg-black p-4"
                    >
                ...
                <div className="mt-6 space-y-3 border-t border-zinc-800 pt-5">
                  ...
                  <div className="flex items-center justify-between text-xl font-bold text-yellow-400">
            ...
            <button
              ...
              disabled={submitting || selectedItemsDetailed.length === 0}
              className="mt-6 w-full rounded-2xl bg-yellow-500 px-4 py-3 font-semibold text-black transition hover:bg-yellow-400 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {submitting ? "Submitting..." : "Continue Submission"}
            </button>
          </aside>
        </section>
      </div>
